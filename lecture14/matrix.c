...
#include <math.h>
#include "matrix.h"
//macros have been defined in matrix.h to simplify code appearance


//MATRICES 

//create a new matrix (init = 0.0)
matrix new_matrix(const int rows, const int cols)
{
    matrix mat;
    mat.rows = rows;
    mat.cols = cols;
    assert(rows>0);//check if dimensions agree 
    assert(cols>0);//check if dimensions agree 
...
{
    assert(mat->rows>0); assert(mat->cols>0);//check if dimensions agree 
    printf("\n %.100s =\n", &varname[1] );
...
matrix matrix_add(const matrix* A, const matrix* B)
{
    ...
    assert(rows==B->rows);//check if dimensions agree 
    assert(cols==B->cols);//check if dimensions agree 
...
matrix matrix_mult(const matrix* A, const matrix* B)
{
    ...
    assert(colsA==rowsB); //check if dimensions agree 
...
//dot multiply each row of A by the vector x
...
double power_iteration(const vector* v, const matrix* A, double TOL, int maxIter) {
    int mstop = 0; int k = 0; 

    //0th iteration
    vector vec = v;
    vector norm = sqrt(vector_dot_mult(v, v)); // magnitude
    //normalize vector
    vget(vec, 1) = vget(vec, 1) / norm;
    vget(vec, 2) = vget(vec, 2) / norm;
    vget(vec, 3) = vget(vec, 3) / norm;
    vget(vec, 4) = vget(vec, 4) / norm;
    vget(vec, 5) = vget(vec, 5) / norm;

    double lambda = vector_dot_mult(matrix_vector_mult(A, vec)), vec);
    

    //kth iteration
    vector w = new_vector(5); // w vector
    vector vec_k = new_vector(5); // k-th vector 
    double lambda_k = 0.0;

    while (mstop == 0) {
        k = k + 1;

        //calculate k-th lambda
        w = matrix_vector_mult(A, vec);
        vec_k = w / sqrt(vector_dot_mult(w, w));
        lambda_k = vector_dot_mult(matrix_vector_mult(A, vec_k)), vec_k);
        

        if (k == maxIter || abs(lambda_k - lambda) < TOL) {
            mstop = 1;
        }
        vec = vec_k;
    }
...
double inverse_power_iteration(double mu, const vector* v, const matrix* A, double TOL, int maxIter) {
    int mstop = 0; int k = 0;

    //create the mu*I matrix 
    matrix I = new_matrix(5);
    for (int i = 1; i <= 5; i++) {
        for (int j = 1; j <= 5; j++) {
            mget(I, i, j) = mu * (i == j);
        }
    ...
    while (mstop == 0) {
        ...
        //calculate k-th lambda
        w = matrix_vector_mult(matrix_sub(A, I), vec);
        vec_k = w / sqrt(vector_dot_mult(w, w));
        lambda_k = vector_dot_mult(matrix_vector_mult(matrix_sub(A, I), vec_k)), vec_k);
