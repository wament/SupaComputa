...
int main()
{
    ...
    for(int i=1; i<=5; i++ ) //row
        for (int j=1; j<=5; j++ ) //col
        {
            ...
            mget(A,i,j) = ((rand()%5) - 3)*(i==1) + 2*(i==5) + 1*(i==j);
            mget(B,i,j) = ((rand()%5) - 3)*(i==1) + 2*(i==5) + 1*(i==j);
        }
    ...
    //approximation test
    //I could not figure out how to invert A for shifted inverse iteration
    double eigen1 = power_iteration(soln, A, 0.00000001, 100);
    print_scalar(eigen1);

    double eigen1 = power_iteration(soln2, A, 0.00000001, 100);
    print_scalar(eigen2);

    double eigen1 = power_iteration(soln3, A, 0.00000001, 100);
    print_scalar(eigen3);
